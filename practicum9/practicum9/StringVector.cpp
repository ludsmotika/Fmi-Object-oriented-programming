...
void StringVector::moveFrom(StringVector&& other) 
{
	...
	this->stringsCount = other.stringsCount;
...
void StringVector::push_back(MyString&& string)
{
	insert(size(), std::move(string));
}

void StringVector::insert(unsigned index, MyString&& string)
{
	...
	{
		strings[i] = std::move(strings[i - 1]);
	}
